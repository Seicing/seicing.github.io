...
function adjustContentWidths() {
    ...
    if (window.innerWidth < 767) {
        ...
        // 为了避免内容跳动，延迟一点点执行
        setTimeout(() => {
            reske.style.height = reski.offsetHeight + 'px';
        }, 50);
    }
}
...
function conditionallyManageLayout() {
    ...
    if (window.innerWidth >= 1280) {
        ...
        // [新增] 在桌面模式下，确保 #reske 内部的 technoseigine 也能被加载
        const reske = document.getElementById('reske');
        if (reske) {
            loadTechnoseigineIfNeeded(reske);
        }
        return;
    }
    ...
    const observer = new MutationObserver((mutationsList, obs) => {
        ...
        for (const mutation of mutationsList) {
            ...
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                console.log("#reske content has been loaded! Cloning now...");
...
(function () {
    document.querySelectorAll('td.dnfmyth').forEach(td => {

        // 如果已经处理过，跳过（防止重复执行）
        if (td.querySelector('.dnfmyth-text')) return;
